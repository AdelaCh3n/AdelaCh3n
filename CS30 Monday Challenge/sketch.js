// CS30 Monday Challenge
// Your Name
// Date
//
// Extra for Experts:
// - describe what you did to take this project "above and beyond"

//global variables
let items = [];
let provinces = new Map();
let currentProv = "SK";
let products = [];

function setup() {
  createCanvas(windowWidth, windowHeight);
  for (let i = 0; i < 20; i++) {//create 20 objects
    let item = {
      x: random(width),
      y: random(height),
      speedY: random(1, 3),
      basePrice: random(10, 100),
      name: "Item " + i
    };
    items.push(item);
  }

  //province map
  provinces.set("SK", { tax: 1.11 });
  provinces.set("AB", { tax: 1.05 });
  provinces.set("ON", { tax: 1.13 });
}

function draw() {
  background(220);
  let rules = provinces.get(currentProv);
  for (let i = 0; i < 20; i++) {
    items[i].y += items[i].speedY;
    if (items[i].y > height) items[i].y = 0;
    //draw rectangle
    rectMode(CENTER);
    textAlign(CENTER, CENTER);
    let totalPrice = items[i].basePrice * rules.tax;

    rect(items[i].x, items[i].y, 60, 40);
    text(items[i].name, items[i].x, items[i].y - 10);
    text("$" + totalPrice.toFixed(2), items[i].x, items[i].y + 10);
  }
}

function keyPressed(){
  if( key === '1')currentProv = "SK";
  if( key === '2')currentProv = "AB";
  if( key === '3')currentProv = "ON";
  if( key === 'x')items.pop();
}